...
export async function resolveGoogleApisCdn(playerUrl: string): Promise<ResolvedMedia> {
...
async function fetchPlayerPage(playerUrl: string, iframe: HTMLIFrameElement): Promise<ResolvedMedia> {
  try {
    ...
    const m3u8Url = baseUrl + '/playlist/' + videoHash + '/playlist.m3u8?token=' + encodeURIComponent(avsToken);
...
async function processEncryptedM3u8(m3u8Text: string, m3u8Headers: Record<string, string>, avsToken: string): Promise<ResolvedMedia> {
  ...
  let cn = '', sk = '', ts = '0', uid = 'anon';

  const envHeader = m3u8Headers['x-envelope'] || m3u8Headers['x-avs-envelope'] || m3u8Headers['x-stream-envelope'] || '';
  if (envHeader) {
    try {
  ...
  const attempts: { unshuffle: { name: string; fn: (s: string) => string }; hmac: { name: string; data: string } }[] = [];
  ...
  for (let idx = 0; idx < attempts.length; idx++) {
    ...
    try {
      ...
      const hmacKey = await crypto.subtle.importKey('raw', cnBytes as unknown as BufferSource, { name: 'HMAC', hash: 'SHA-256' }, false, ['sign']);
      const gcmKeyBuf = await crypto.subtle.sign('HMAC', hmacKey, hmacData as unknown as BufferSource);
      const gcmKey = await crypto.subtle.importKey('raw', gcmKeyBuf as unknown as BufferSource, { name: 'AES-GCM' }, false, ['decrypt']);
      const rawResult = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: iv as unknown as BufferSource }, gcmKey, encryptedBlob as unknown as BufferSource);

      const rawBytes = new Uint8Array(rawResult);
      let m3u8Body = new TextDecoder().decode(rawBytes);

      if (m3u8Body.indexOf('#EXTINF') === -1 && m3u8Body.indexOf('/hls/') === -1) {
        const descrambled = descramble(rawBytes, sk, ts);
        m3u8Body = new TextDecoder().decode(descrambled);
      }

      let fullM3u8Text = headerLines.join('\n') + '\n' + m3u8Body;
      if (fullM3u8Text.indexOf('#EXT-X-ENDLIST') === -1) fullM3u8Text += '\n#EXT-X-ENDLIST';
      ...
      if (hasEncryptedUrls) {
        debugLog('Layer 2: decrypting ' + fullM3u8Text.split('\n').length + ' lines');
      ...
      if (fullM3u8Text.indexOf('#EXTINF') !== -1) {
        ...
        const blob = new Blob([fullM3u8Text], { type: 'application/vnd.apple.mpegurl' });
...
async function decryptSegmentUrls(intermediateM3u8: string, headerLines: string[], jtiOdd: string): Promise<ResolvedMedia> {
  ...
  async function deriveCtrKey(fileId: string): Promise<CryptoKey> {
    ...
    const k = await crypto.subtle.importKey('raw', hmacKeyBytes as unknown as BufferSource, { name: 'HMAC', hash: 'SHA-256' }, false, ['sign']);
    const buf = await crypto.subtle.sign('HMAC', k, signData as unknown as BufferSource);
    const ctrKey = await crypto.subtle.importKey('raw', buf as unknown as BufferSource, { name: 'AES-CTR' }, false, ['decrypt']);
  ...
  const promises = segments.map(async (seg) => {
    ...
    const dec = await crypto.subtle.decrypt({ name: 'AES-CTR', counter: counter as unknown as BufferSource, length: 64 }, ctrKey, encrypted as unknown as BufferSource);
