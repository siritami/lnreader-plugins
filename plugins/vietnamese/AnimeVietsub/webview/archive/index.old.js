    ...
    function tryAttempt(idx) {
      ...
      crypto.subtle
        ...
        .then(function (rawResult) {
          ...
          // Site's new loader removed descramble. Check if it's already valid plaintext:
          var m3u8Body = new TextDecoder().decode(rawBytes);

          // Fallback to descramble if it doesn't look like valid m3u8/hls paths
          if (
          ...
          if (m3u8Text.indexOf('#EXT-X-ENDLIST') === -1)
            m3u8Text += '\n#EXT-X-ENDLIST';
          console.log('m3u8 file:', m3u8Text); // hidden log, Electron only
